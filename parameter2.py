...
st.title("$x_2$の初期値を固定して$x_1$の初期値を変動させたときのそれに伴う宇宙進化の変動")
# --- λ 設定 ---
...
for i, x1_0 in enumerate(x1_0_list):
    for j, x2_0 in enumerate(x2_0_list):
        ...
        ax.plot(shifted_z, Omega_arr_r, label=label, color=color)
        ax.plot(shifted_z, Omega_arr_m, label=label, color=color)
        ax.plot(shifted_z, Omega_arr_DE, label=label, color=color)
        labels.append(label)

        # z=0に最も近いインデックスを取得して出力
        index_z0 = np.argmin(np.abs(np.array(Omega_DE_list) - 0.68))
        Omega_DE_z0 = Omega_arr_DE[index_z0]
        Omega_r_z0 = Omega_arr_r[index_z0]

        st.write(f"x1_0={x1_0:.0e}, x2_0={x2_0:.0e} のときの Ω_phi(z=0) = {Omega_DE_z0:.4f}")
        st.write(f"x1_0={x1_0:.0e}, x2_0={x2_0:.0e} のときの Ω_r(z=0) = {Omega_r_z0:.4f}")
ax.set_xscale("log")
...
st.title("$x_1$の初期値を固定して$x_2$の初期値を変動させたときのそれに伴う宇宙進化の変動")

# --- 初期値のパターン ---
x1_0_list = [1e-10]
x2_0_list = [1e-11, 1e-10, 1e-9]
#x2の初期値を固定してx1の初期値を変動させると、グラフは微動だにしない。しかし、x1の初期値を固定してx2の初期値を変動させると、グラフはめっちゃ動く。
#めっちゃ動くのはOmega_r=Omega_mとなるzが変動することにつながる。
z_0 = 2.5 * 10**6
Omega_r0 = 0.9989
